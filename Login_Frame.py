...
class PlaceholderEntry(ttk.Entry):
    ...
    def _cursor(self, *args):  # method to not allow user to move cursor when placeholder exists
        if self.get() == self.text and self.__has_placeholder:
            self.icursor(0)
# ##########################################
user_lbl= Label(frame, text  = "Enter Name: ", width=23, foreground='#F4F6F7', background='#880B2D', font="Arial 22 bold")
pass_lbl= Label(frame, text  = "Enter Password: ", width=23, foreground='#F4F6F7', background='#880B2D', font="Arial 22 bold")
user_entry= PlaceholderEntry(frame,placeholder='Enter Username', placeholdercolor="#C0C0C0",width=30,  font='Arial 20 bold')
pass_entry= PlaceholderEntry(frame,placeholder='Enter Password', show='*', width=30, font='Arial 20 bold')
...
def login_fun():
    #print(type(user_entry.get()))
    cr.execute(f'select * from sign_in where lower(username)="{user_entry.get().lower()}" and password={pass_entry.get()}')
    if(cr.fetchone()):
        root.destroy()
        os.startfile("C:/Users/acer/Downloads/FRAMEs/home.py")
    else: messagebox.showerror("Error", "Staff access only!!".title())
def mang():
    root.destroy()
...
def exit():
    ...
    if answer:
        root.destroy()
lbl=Label(frame,text="Login Frame",width=35, background='#5D2D07', font="Arial 40 bold",justify='center', foreground='white',padding=(10, 5), anchor='center').place(x=280,y=70)
